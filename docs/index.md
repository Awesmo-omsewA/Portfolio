# Привет, меня зовут Андрей 👋

Я пишу техническую документацию. Это портфолио — мой путь, решения и проекты, которые я делал с нуля. Не готовый итог, а процесс, который продолжается.

---

## Что вы найдёте здесь
- **[Как я создавал это портфолио](process/)** — процесс выбора инструментов, настройки и публикации.
- **[Мои проекты](projects/)** — примеры документации для реальных и учебных проектов.
- **[Обо мне](about.md)** — мой опыт, навыки и контакты.

---

## Инструменты, с которыми работаю

- VS Code, Markdown
- Git, GitHub
- MkDocs (Material for MkDocs)
- Swagger / OpenAPI
- GitHub Actions (CI/CD)


---


#### [GitHub-репозиторий](https://github.com/Awesmo-omsewA/Portfolio)
  

---


📬 Связаться со мной можно через [GitHub](https://github.com/Awesmo-omsewA) или по почте: [Andrejsonnyj@gmail.com](mailto:Andrejsonnyj@gmail.com)